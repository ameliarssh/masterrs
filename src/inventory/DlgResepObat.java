  ...
  isForm();                
    ...
    private void ppLembarObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObatActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("tanggallahir",Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText()))); 
            param.put("nomertelp",Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText()))); 
